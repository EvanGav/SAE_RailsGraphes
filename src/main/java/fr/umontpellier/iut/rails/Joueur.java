...
public class Joueur {
    ...
    private int score;
    /**
     * Graphe representant le plateau avec les routes capturées par le joueur
     */
    private Graphe routesCaputre;
    ...
    public Joueur(String nom, Jeu jeu, Couleur couleur) {
        ...
        routesCaputre=new Graphe(jeu.getVilles().size());
    }
    ...
    public Graphe getRoutesCaputre() {
        return routesCaputre;
    }
    ...
    private int indexVille(Ville ville){
        return jeu.getVilles().indexOf(ville);
    }
    ...
    public void capturerRoute(Route route) {
        if (route.capturerParJoueur(this)) {
            ...
            routesCaputre.ajouterArete(indexVille(route.getVille1()),indexVille(route.getVille2()),n);
            nbWagons -= n;
            switch (n) {
                case 1 -> score += 1;
                case 2 -> score += 2;
    ...
    private Ville getVilleParNom(String nom) {
        for (Ville ville : jeu.getVilles()) {
    ...
    public boolean possedeVoisinGare(Ville v){
        for(ArrayList<Integer> a : routesCaputre.calculerClassesDeConnexite()){
            for(int i : jeu.getGraphe().voisins(indexVille(v))){
                if(a.contains(i)){
                    return true;
                }
            }
    ...
    private boolean destinationReussie(Destination d){
        int i=indexVille(getVilleParNom(d.getVille1()));
        int j=indexVille(getVilleParNom(d.getVille2()));
        for(ArrayList<Integer> a : routesCaputre.calculerClassesDeConnexite()){
            if(a.size()>1){
                if(a.contains(i) && a.contains(j)){
                    return true;
                }
                else if(a.contains(i) && (getVilleParNom(d.getVille2()).getProprietaire()!=null && getVilleParNom(d.getVille2()).getProprietaire().equals(this))){
                    if(possedeVoisinGare(getVilleParNom(d.getVille2()))){
                        return true;
                    }
                }
                else if(a.contains(j) && (getVilleParNom(d.getVille1()).getProprietaire()!=null && getVilleParNom(d.getVille1()).getProprietaire().equals(this))){
                    if(possedeVoisinGare(getVilleParNom(d.getVille1()))){
                        return true;
                    }
                }
            }
    ...
    public int scoreDestinations(){
        int score=0;
        for(Destination d : destinations){
            if (destinationReussie(d)){
                score+=d.getValeur();
            }
            else{
                score-=d.getValeur();
