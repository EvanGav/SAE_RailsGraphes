...
public class Jeu implements Runnable {
    ...
    public Jeu(String[] nomJoueurs) {
        ...
        g=new Graphe(villes.size());
        for(Route r : routes){
            g.ajouterArete(villes.indexOf(r.getVille1()),villes.indexOf(r.getVille2()),r.getLongueur());
        }
    }
    ...
    private ArrayList<Joueur> plusLongs(){
        Graphe g= joueurs.get(0).getRoutesCaputre();
        ArrayList<Joueur> j=new ArrayList<>();
        j.add(joueurs.get(0));
        for(int i=1;i<joueurs.size();i++){
            if(g.poidCheminPlusLong()<joueurs.get(i).getRoutesCaputre().poidCheminPlusLong()){
                g=joueurs.get(i).getRoutesCaputre();
                j.clear();
                j.add(joueurs.get(i));
            }
            else if(g.poidCheminPlusLong()==joueurs.get(i).getRoutesCaputre().poidCheminPlusLong()){
                j.add(joueurs.get(i));
            }
        }
        return j;
    ...
    public List<Integer> calculerLesScores() {
        ...
        for(Joueur j : plusLongs()){
            j.setScore(10);
        }
        for(Joueur j : joueurs){
            j.setScore(j.scoreDestinations());
